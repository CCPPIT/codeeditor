...
import SnippetsPageSkeleton from '@/components/snippet/SnippetsPageSkeleton';
import { SnippetCard } from '@/components/snippet/SnippetCard';



const SnippetPage = () => {
// استعلام للحصول على المقتطفات
const snippets = useQuery(api.snippets.getSnippets); 
// تعريف الحالة لبحث المستخدم
const [searchQuery, setSearchQuery] = useState(""); 
// تعريف الحالة للغة المحددة
const [selectedLanguage, setSelectedLanguage] = useState<string | null>(null); 

const [view,setView]=useState<"grid"|"list">("grid")
// التحقق مما إذا كانت المقتطفات غير محددة
if (snippets === undefined) {
    return (
      <div className='min-h-screen'>
        <NavigationHeader /> {/* عرض رأس التنقل */}
        <SnippetsPageSkeleton/>
      </div>
    );
}

// استخراج اللغات الفريدة من المقتطفات
const languages = [...new Set(snippets.map((s) => s.language))];
// اختيار أكثر 5 لغات شيوعاً
const popularLanguages = languages.slice(0, 5);

// تصفية المقتطفات بناءً على استعلام البحث واللغة المحددة
const filteredSnippets = snippets.filter((snippet) => {
    const matchesSearch =
        snippet.title.toLowerCase().includes(searchQuery.toLowerCase()) || // تحقق من تطابق العنوان
        snippet.language.toLowerCase().includes(searchQuery.toLowerCase()) || // تحقق من تطابق اللغة
        snippet.userName.toLowerCase().includes(searchQuery.toLowerCase()); // تحقق من تطابق اسم المستخدم
    
    const matchesLanguage = !selectedLanguage || snippet.language === selectedLanguage; // تحقق من تطابق اللغة المحددة
    
    return matchesSearch && matchesLanguage; // إرجاع المقتطفات التي تطابق الشروط
});
return (
