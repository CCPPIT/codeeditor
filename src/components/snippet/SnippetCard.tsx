...
export const SnippetCard = ({snippet}: Props) => {
 // استيراد البيانات من سياق المستخدم
const { user } = useUser();
// استخدام دالة الحذف من API
const deleteSnippet = useMutation(api.snippets.deleteSnippet);
// تعريف حالة لتتبع حالة الحذف
const [isDeleting, setIsDeleting] = useState(false);

// دالة معالجة الحذف
const handleDelete = async () => {
    // تعيين حالة الحذف إلى true
    setIsDeleting(true);
    try {
        // محاولة حذف المقتطف
        ...
        // تسجيل الخطأ في وحدة التحكم وعرض رسالة خطأ
        console.log("Error deleting snippet", error);
        toast.error("Error deleting snippet");
    } finally {
        // إعادة تعيين حالة الحذف إلى false
        setIsDeleting(false);
    }
};
return (
